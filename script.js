function calculate() {
    alert("Calculate button clicked!"); // Confirm function runs
    ...
    alert(`Inputs: ${aa}, ${bb}, ${cc}, ${dd}, ${vv}, ${xx}, ${yy}, ${zz}, ${oo}`); // Debug inputs
    ...
    // Validate range
    if (a > b || c > d) {
        alert("Invalid input range. Ensure (aa,bb) <= (cc,dd) and (vv,xx) <= (yy,zz).");
        return;
    }
    ...
    alert(`Selected Checkboxes: ${selected_values.join(", ")}`); // Debug selected values
    ...
    let tableHTML = `<table border="1" cellspacing="0" cellpadding="5" style="width: 100%; text-align: center;">
                        <tr>
                            <th>#</th>
                            <th>Dimensions</th>
                            <th>Area (sq.ft)</th>`;
    ...
    selected_values.forEach(id => {
        tableHTML += `<th>${id.toUpperCase()}</th>`;
    });
